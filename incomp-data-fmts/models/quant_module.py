...
class QuantPaCTActiv(nn.Module):
    ...
    def __init__(self, bits, max_inp_val=6.0):
        ...
        for bit in self.bits:
            self.mix_activ.append(
                LearnedClippedLinearQuantization(
                    num_bits=bit, init_act_clip_val=max_inp_val
                )
            )
    ...
    def __init__(self, inplane, outplane, bits, **kwargs):
        ...
        for bit in self.bits:
            ...
            self.mix_bias.append(
                FQConvBiasQuantization(outplane, num_bits=bit, abit=self.abits)
            )
...
class FpConv2d(nn.Module):
    ...
    def __init__(self, inplane, outplane, wbits, abits, **kwargs):
        ...
        self.relu = nn.ReLU()
        # complexities
        stride = kwargs["stride"][0] if "stride" in kwargs else 1
        if isinstance(kwargs["kernel_size"], tuple):
            kernel_size = kwargs["kernel_size"][0] * kwargs["kernel_size"][1]
        else:
            kernel_size = kwargs["kernel_size"] * kwargs["kernel_size"]
    ...
    def forward(self, input):
        ...
        if not self.first_layer and not self.fc:
...
class MixQuantPaCTActiv(nn.Module):
    ...
    def __init__(self, bits, max_inp_val=6.0, gumbel=False):
        super().__init__()
        self.bits = bits
        self.gumbel = gumbel
