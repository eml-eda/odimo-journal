...
def main(rank, world_size, port, args):
    ...
    model = model_fn(
        args.arch_cfg, num_classes=1000,
        input_size=224, fine_tune=args.fine_tune, std_head=True)
    model = model.to(rank)
    ...
    for epoch in range(last_epoch, N_EPOCHS):
        # train for one epoch
        train(train_dl, model, criterion, optimizer, q_optimizer, epoch, rank, args)
...
def train(train_loader, model, criterion, optimizer, q_optimizer, epoch, device, args):
