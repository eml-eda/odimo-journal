...
from utils import get_free_port, ddp_setup, DDPCheckPoint
...
def resume_ckp(ckp, model, test_dl, rank):
    ...
    arch_optimizer_state_dict = ckp['optimizer_state_dict_1']
    q_optimizer_state_dict = ckp['optimizer_state_dict_2']
    ...
    return (model, optimizer_state_dict, arch_optimizer_state_dict,
            q_optimizer_state_dict, last_epoch)
...
def main(rank, world_size, port, args):
    ...
    # Get the Data
    data_dir = DATA_DIR
    datasets = imn.get_data(data_dir=data_dir, val_split=VAL_SPLIT, seed=args.seed)
    ...
    if args.resume_ckp is not None:
        model, opt_sd, arch_opt_sd, q_opt_sd, last_epoch = \
    ...
    if arch_opt_sd is not None:
        arch_optimizer.load_state_dict(arch_opt_sd)
    ...
    is_best = False
    checkpoint = DDPCheckPoint(CHECKPOINT_DIR, model,
    ...
    for epoch in range(last_epoch, N_EPOCHS):
        # train for one epoch
        train(train_dl, model, criterion, optimizer, arch_optimizer,
              q_optimizer, epoch, rank, TEMP,
              args)
        ...
        if hasattr(model, 'module'):
            ...
            best_arch, cycles, bita, bitw, mixcycles, mixbita, mixbitw = outs
        logging.info('best model with {}: {:.3f}M, bita: {:.3f}K, bitw: {:.3f}M'.format(
            args.target, cycles, bita, bitw))
        logging.info('expected model with {}: {:.3f}M, bita: {:.3f}K, bitw: {:.3f}M'.format(
        ...
        if val_dl is not None:
            acc1 = validate(val_dl, model, criterion, epoch, rank, TEMP, args)
        acc1_test = validate(test_dl, model, criterion, epoch, rank, TEMP, args)
...
def train(train_loader, model, criterion,
          optimizer, arch_optimizer, q_optimizer, epoch, device, temp,
          args):
...
def validate(val_loader, model, criterion, epoch, device, temp, args):
    batch_time = AverageMeter('Time', ':6.3f')
    losses = AverageMeter('Loss', ':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')
    top5 = AverageMeter('Acc@5', ':6.2f')
...
if __name__ == '__main__':
    ...
    if args.checkpoint_dir is None:
        ...
        args.checkpoint_dir = args.checkpoint_dir / str(args.arch)[3:]
