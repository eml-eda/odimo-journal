...
parser.add_argument(
    "--input-res", type=int, default=None, help="Input Resolution (used only for res18)"
)
parser.add_argument(
    "--no-std-head",
    default=True,
...
parser.add_argument(
    "--analog-speedup", type=float, default=5.0, help="SpeedUp of analog wrt digital"
)
parser.add_argument(
    "--pretrained-model", type=str, default=None, help="Pretrained model path"
)
...
if args.pretrained_model is not None:
    if "mix" in args.arch:
        ...
        model = models.__dict__[args.arch](args.pretrained_model, target=args.target)
    else:
        if model_name == "res18":
            model = models.__dict__[args.arch](
                args.pretrained_model,
                num_classes=args.num_classes,
                fine_tune=False,
                analog_speedup=args.analog_speedup,
    ...
    if model_name == "temponet":
        model = models.__dict__[args.arch]("ppg/warmup_fp.pth.tar", target=args.target)
    else:
        model = models.__dict__[args.arch](
