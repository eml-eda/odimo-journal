...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--init-strategy", type=str, help='One of {"1st", "2nd", "half"}'
    )
    parser.add_argument(
        "--strength", type=float, default=0.0e0, help="Regularization Strength"
    )
    parser.add_argument(
        "--pretrained-model", type=str, default=None, help="Path to pretrained model"
    )
    parser.add_argument(
        "--warmup", default=False, action="store_true", help="Whether to perform warmup"
    )
    parser.add_argument(
        "--warmup-strategy", type=str, help='One of {"std", "coarse", "fine"}'
    )
    parser.add_argument(
        "--cost", type=str, default="naive", help='One of {"naive", "darkside"}'
    )
    parser.add_argument(
        "--search", default=False, action="store_true", help="Whether to perform search"
    )
    parser.add_argument(
        "--finetune",
        default=False,
