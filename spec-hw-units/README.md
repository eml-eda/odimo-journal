...
At the end of the search phase, the mapping is discretized and the final discovered network is trained *from-scratch* by optimizing only the task loss $\mathcal{L}$ with respect to the weights $W$. Differently for DIANA, where fine-tuning was sufficient, in this case, we found that training from scratch is beneficial.
